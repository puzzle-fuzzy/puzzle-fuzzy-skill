...
不要一次性“全局替换”，但也不要把已经确认的问题拖到后续迭代。推荐按以下顺序逐项目迁移：

迁移策略的核心是“尽早统一，避免双重维护”：

- 发现目录、工具链、env、共享 UI、测试或脚本位置需要调整时，默认在当前迁移阶段一次性切换，并同步更新调用方、文档、CI、Docker 和测试。
- 不为了保守而长期保留旧目录/新目录、旧配置/新配置、旧命令/新命令或旧 UI/新 UI 两套实现；双轨会扩大测试矩阵、增加认知成本，并让后续删除更危险。
- 临时兼容层只用于不可逆数据迁移、外部公开契约、已发布客户端或无法立即切换的部署边界。兼容层必须同时记录原因、影响范围、删除条件、负责人和目标日期。
...
8. **立即清理兼容层**：只保留经过确认的临时兼容层；满足删除条件后在同一迁移阶段删除旧变量、旧路径、旧目录和旧实现，并保留迁移说明和验证证据。
